...
export interface DishTableItem {
  name: string;
  id: number;
  price?: number; // ? si on veut que le prix soit facultatif. donc si le prix existe, alors il est un nombre.
}
...
export class DishTableDataSource extends DataSource<DishTableItem> {
   ...
   * this would be replaced by requesting the appropriate data from the server.
   */
  private getSortedData(data: DishTableItem[]) { // méthode qui permet d'obtenir les objets triés
    ...
    return data.sort((a, b) => {
      ...
      switch (this.sort.active) {
        ...
        case 'id': return compare(+a.id, +b.id, isAsc); // l'opérateur "+" permet de caster : convertir un string en number
